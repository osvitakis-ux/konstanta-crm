...
function telGetLog(){
  try{ return JSON.parse(localStorage.getItem(TEL_LOG_KEY)||'[]'); }catch(e){ return []; }
}
function telAddLog(entry){
  var log = telGetLog();
  log.unshift(Object.assign({id:uid(), ts: new Date().toISOString()}, entry));
  if(log.length > 500) log = log.slice(0,500);
  localStorage.setItem(TEL_LOG_KEY, JSON.stringify(log));
}
...
function renderTelephony(){
  ...
  if(whEl) whEl.value = window.location.origin + '/webhook/telephony';
...
function renderTelLog(){
  var body = document.getElementById('tel-log-body');
  if(!body) return;
  var filter = (document.getElementById('tel-log-filter')||{value:''}).value;
  var log = telGetLog().filter(function(e){ return !filter || e.direction===filter; });

  if(!log.length){
    body.innerHTML = '<div class="empty"><div class="ei">📞</div>Журнал дзвінків порожній</div>';
    return;
  }
  ...
  body.innerHTML = '<table style="width:100%;border-collapse:collapse">'
    ...
    + '</tr></thead><tbody>'
    + log.map(function(e){
        var s = e.studentId ? (S.students||[]).find(function(x){return x.id===e.studentId;}) : null;
        var sName = s ? s.fn+' '+s.ln : (e.callerName||'—');
        ...
        return '<tr style="border-top:1px solid var(--b1);font-size:13px">'
          +'<td style="padding:8px;color:'+dirClr[e.direction||'inbound']+'">'+dirIcon[e.direction||'inbound']+' '+dirLbl[e.direction||'inbound']+'</td>'
          ...
          +'<td style="padding:8px">'+(e.recordUrl
            ? '<audio controls style="height:28px;max-width:180px"><source src="'+e.recordUrl+'"></audio>'
            : '<span style="color:var(--t3);font-size:11px">немає</span>')+'</td>'
          +'<td style="padding:8px">'
            +(e.studentId?'':'<button class="btn btn-g btn-sm" onclick="telLinkStudent(\''+e.id+'\',\''+e.phone+'\')">Прив\'язати</button>')
          +'</td>'
          +'</tr>';
      }).join('')
    + '</tbody></table>';
...
function telLinkStudent(logId, phone){
  ...
  if(found){
    ...
    if(entry){ entry.studentId=found.id; entry.callerName=found.fn+' '+found.ln; }
    localStorage.setItem(TEL_LOG_KEY, JSON.stringify(log));
    mkToast('Прив\'язано до '+found.fn+' '+found.ln);
    renderTelLog();
  } else {
...
// Called by webhook (when provider sends event to your backend → your backend calls this)
function telHandleEvent(event){
  var cfg = telGetSettings();
  // event: { type:'call_start'|'call_end'|'missed', phone, direction, duration, recordUrl }
  if(event.type === 'missed' || event.type === 'call_end'){
    var student = (S.students||[]).find(function(s){
      return s.phone && s.phone.replace(/\D/g,'') === (event.phone||'').replace(/\D/g,'');
    });
    var logEntry = {
      direction:  event.direction||'inbound',
      phone:      event.phone||'',
      studentId:  student?student.id:null,
      callerName: student?(student.fn+' '+student.ln):'',
      duration:   event.duration||0,
      recordUrl:  event.recordUrl||null,
    };
    telAddLog(logEntry);
    // Auto-log to communications
    if(cfg.autolog && student){
      dbInsert('comms',{
        id: uid(),
        ...
        notes: (event.direction==='inbound'?'Вхідний':'Вихідний')+' дзвінок'
          +(event.duration?' ('+Math.floor(event.duration/60)+'хв '+event.duration%60+'с)':'')
          +(event.type==='missed'?' — ПРОПУЩЕНИЙ':''),
        branch_id: myBranchId()||null,
      }).catch(function(){});
    }
    // Popup notification
    if(cfg.popup && event.type !== 'call_end'){
      telShowPopup(event.phone, student);
    }
    if(document.getElementById('pg-telephony')&&document.getElementById('pg-telephony').classList.contains('active')){
      renderTelLog();
    }
  }
}

function telShowPopup(phone, student){
  var old = document.getElementById('tel-call-popup');
  if(old) old.remove();
  var pop = document.createElement('div');
  pop.id = 'tel-call-popup';
  pop.style.cssText = 'position:fixed;bottom:24px;right:24px;background:var(--s1);border:2px solid var(--adm);border-radius:14px;padding:16px 20px;z-index:99999;box-shadow:0 8px 32px rgba(0,0,0,.25);min-width:260px;animation:slideUp .3s ease';
  pop.innerHTML = '<div style="display:flex;align-items:center;gap:12px">'
    +'<div style="font-size:28px;animation:ring 1s infinite">📞</div>'
    +'<div><div style="font-weight:700;font-size:14px">'+(student?student.fn+' '+student.ln:'Невідомий абонент')+'</div>'
    +'<div style="font-size:12px;color:var(--t2);font-family:JetBrains Mono,monospace">'+phone+'</div>'
    +(student?'<div style="font-size:11px;color:var(--tut);margin-top:2px">'+( student.subject||student.subj||'')+'</div>':'')
    +'</div>'
    +'<button onclick="this.closest(\'#tel-call-popup\').remove()" style="margin-left:auto;background:none;border:none;font-size:18px;cursor:pointer;color:var(--t3)">×</button>'
    +'</div>'
    +(student?'<button class="btn btn-p btn-sm" style="margin-top:10px;width:100%" onclick="nav(\'students\');this.closest(\'#tel-call-popup\').remove()">Відкрити картку учня</button>':'');
  document.body.appendChild(pop);
  setTimeout(function(){ if(pop.parentNode) pop.remove(); }, 30000);
}

window.renderTelephony = renderTelephony;
window.telProviderChange = telProviderChange;
window.telSaveSettings = telSaveSettings;
window.telTestConn = telTestConn;
window.telCopyWebhook = telCopyWebhook;
window.renderTelLog = renderTelLog;
window.telLinkStudent = telLinkStudent;
window.telHandleEvent = telHandleEvent;window.SupabaseMini = (function(){
...
var ROLES = {
  ...
  admin: {
    label:'\u0410\u0434\u043C\u0456\u043D\u0456\u0441\u0442\u0440\u0430\u0442\u043E\u0440', icon:'\uD83D\uDEE1\uFE0F', color:'var(--adm)',
    avatarBg:'linear-gradient(135deg,#29abe2,#3fa9f5)',
    nav:['dashboard','students','tutors','schedule','lessons','comms','analytics','crm','invoice','reports'],
    can:{students:true,tutors:true,lessons:true,comms:true,payments:false,users:false,settings:false,danger:false,deleteAny:true},
    seeIncome:false, seeAll:true, canEditUsers:false, showGodBanner:false
  },
  network_admin: {
